...
from src.directory_docs.models import dir_cession, docs_cession, dir_credit, dir_folder
...
async def get_file_credit(credit_id: int = None, session: AsyncSession = Depends(get_async_session)):
    try:
        if credit_id:
            cession_id_query = await session.execute(select(credit.c.cession_id).where(credit.c.id == credit_id))
            cession_id: int = cession_id_query.scalar()
...
async def save_file_credit(credit_file: UploadFile, file_name: str, cession_id: int = None, session: AsyncSession = Depends(get_async_session)):
