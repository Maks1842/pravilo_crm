...
async def extract_definition(files: List[UploadFile] = File(...), session: AsyncSession = Depends(get_async_session)):
    ...
    type_doc = 'ПСТ'
    re_pattern = RePattern.heading_res
...
def data_executing_doc(type_doc_file, directory_result):
    ...
    for file in filelist:
        ...
        text_debtor = re.search(RePattern.text_debtor, str(text)).group()
        try:
            text_resolution = re.search(RePattern.text_resolution, str(text)).group()
        except:
        ...
        # СПИ
        try:
            tribunal = re.search(RePattern.bailiff, str(text)).group()
            if len(tribunal) > 200:
        ...
        # Взыскатель
        try:
            name_creditor = re.search(RePattern.name_cred_res, str(text)).group()
        except:
        ...
        # Резолюция
        try:
            resolution = re.search(RePattern.resolution_res, str(text_resolution)).group()
        except:
            resolution = 'ОШИБКА'
        ...
        file_newname = os.path.join(f'{directory_result}/', f'{count_doc}_{type_doc_file} от {date_doc}_{name_file_debtor}_{page_count}.pdf')
