...
async def extract_tribunal_writ(file_object: UploadFile, session: AsyncSession = Depends(get_async_session)):

    type_doc = 'СП'
    re_pattern = RePattern.heading_sp

    with open(f'{path_main}/src/media/reader/tribunal_writ_scan.pdf', 'wb+') as f:
        for chunk in file_object.file:
            f.write(chunk)

    path_file = f'{path_main}/src/media/reader/tribunal_writ_scan.pdf'

    extract_items = data_executing_doc(type_doc, path_file, re_pattern)
    ...
    for item in extract_items:
        ...
        data_items.append({
            ...
            "directoryItem": item['directory_result'],
        })
...
def data_executing_doc(type_doc_file, path_file, re_pattern):
    directory_result = split_into_pages(type_doc_file, path_file, re_pattern)
    ...
    for file in filelist:
        ...
        try:
            ...
            name_file_num_case = re.search(r'[\d\-]*(?=/)', num_doc).group()
        except:
            num_doc = 'ОШИБКА'
        ...
        # Сумма долга
        try:
            summa_debt = re.sub(r'\s+', '', re.search(RePattern.summa_rub, str(text_debt)).group())
        except:
            try:
                summa_debt = re.sub(r'\s+', '', re.sub(r'\s*\([\D\s]*|\s*руб\.\s*|\s*рубля\s*', ',', re.search(RePattern.summa_rub2, str(text_debt)).group()))
            except:
                summa_debt = 'ОШИБКА'

        # Сумма пошлины
        try:
            summa_duty = re.sub(r'\s+', '', re.search(RePattern.summ_duty1, str(text_duty)).group())
        except:
            try:
                summa_duty = re.sub(r'\s+', '', re.sub(r'\s*\([\D\s]*|\s*руб\.\s*|\s*руб\s*|\s*рубля\s*', ',', re.search(RePattern.summ_duty2, str(text_duty)).group()))
            except:
                summa_duty = 'ОШИБКА'
        ...
        # debug_log.debug(f'{file_newname=}')
